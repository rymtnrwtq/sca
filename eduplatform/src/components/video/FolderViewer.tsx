...
import { ChevronRight, SlidersHorizontal, EyeOff, Bookmark, ChevronUp, ChevronDown, ExternalLink, FileText, Folder } from 'lucide-react';
import { SeminarLink } from '../../constants';
import { Lesson } from '../../types';
import { SortKey, SortDir, SORT_OPTIONS } from '../../constants';
import {
  getWatchProgress,
  isInWatchLater,
  loadHiddenVideos
...
export const FolderViewer = ({
  ...
  const storageKey = `sca_folder_${folderId}_prefs`;
  ...
  const [statusFilter, setStatusFilter] = useState<StatusFilter>(savedPrefs.statusFilter ?? 'all');
  const [sort, setSort] = useState<SortKey>(savedPrefs.sort ?? 'date');
  const [sortDir, setSortDir] = useState<SortDir>(savedPrefs.sortDir ?? 'desc');
  const [isFilterOpen, setIsFilterOpen] = useState(false);
  const [page, setPage] = useState<number>(savedPrefs.page ?? 1);
  ...
  const handleSort = (key: SortKey) => {
    ...
    else { setSort(key); setSortDir(key === 'title' ? 'asc' : 'desc'); }
  };

  useEffect(() => {
    localStorage.setItem(storageKey, JSON.stringify({ statusFilter, page, sort, sortDir }));
  }, [statusFilter, page, sort, sortDir, storageKey]);

  useEffect(() => {
    setLoading(true);
    const pid = projectId ? `&project_id=${projectId}` : '';
    fetch(`/api/folder-videos?folder_id=${folderId}${pid}&per_page=500`)
  ...
  const sorted = [...filtered].sort((a, b) => {
    ...
    if (sort === 'duration') cmp = (a.durationSec ?? 0) - (b.durationSec ?? 0);
    if (sort === 'title')    cmp = a.title.localeCompare(b.title, 'ru');
  ...
  useEffect(() => {
    if (page > displayTotalPages) setPage(1);
    ...
    <div className="space-y-6 overflow-x-hidden">
      <div className="flex flex-col gap-5">
        <div className="flex items-center justify-between gap-4">
          <button onClick={onBack} className="flex items-center gap-2 text-orange-500 hover:text-orange-400 text-sm font-bold transition-colors w-fit">
